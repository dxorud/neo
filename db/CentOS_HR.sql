...
SELECT count(*) FROM usertbl;ALTER 
...
SELECT concat('이것이', 'OLACLE이다'), '이것이' || 'OLACLE이다' FROM dual;

SELECT instr('이것이 OLACLE이다. 이것도 OLACLE이다', '이것', 2) FROM dual;

SELECT lower('abcdEFGH'), upper('abcdEFGH'), initcap('this is oracle') FROM dual;

SELECT REPLACE('이것이 OLACLE이다', '이것이', 'This is') FROM dual;

SELECT translate('이것이 OLACLE이다', '이것이', 'AB') FROM dual;
...
SELECT next_day('2025-03-10', '금요일'), next_day(sysdate, '토요일') FROM dual;

SELECT months_between(sysdate, '2001-03-12') FROM dual;
